...
export const BackupRestorePanel: React.FC<BackupRestorePanelProps> = ({ theme, userRole }) => {
  ...
  const surface = isDark ? 'bg-[#15171C] border-[#1F2229] text-white' : 'bg-white border-slate-200 text-slate-900 shadow-sm';
  const muted = isDark ? 'text-[#8E9299]' : 'text-slate-500';
  ...
  return (
    <section className={`max-w-4xl mx-auto rounded-2xl border overflow-hidden ${surface}`}>
      <div className={`p-6 border-b ${isDark ? 'border-[#1F2229]' : 'border-slate-200'}`}>
      ...
      <div className="p-6 grid gap-5 md:grid-cols-2">
        <div className={`p-5 rounded-xl border ${isDark ? 'border-[#2A2E38] bg-[#0D0F14]' : 'border-slate-200 bg-slate-50'}`}>
        ...
        <div className={`p-5 rounded-xl border ${isDark ? 'border-[#2A2E38] bg-[#0D0F14]' : 'border-slate-200 bg-slate-50'}`}>
          ...
          <label className={`mt-3 flex items-center gap-2 p-3 rounded-lg border border-dashed cursor-pointer text-xs ${isDark ? 'border-[#3A3F4A] hover:bg-[#15171C]' : 'border-slate-300 hover:bg-white'}`}>
          ...
          <div className="flex gap-2 mt-3">
            <label className={`flex-1 text-xs p-2 rounded-lg border cursor-pointer ${mode === 'merge' ? 'border-cyan-500 text-cyan-500' : isDark ? 'border-[#2A2E38]' : 'border-slate-200'}`}>
              <input type="radio" checked={mode === 'merge'} onChange={() => setMode('merge')} className="mr-2" /> Mesclar
            </label>
            <label className={`flex-1 text-xs p-2 rounded-lg border cursor-pointer ${mode === 'replace' ? 'border-rose-500 text-rose-500' : isDark ? 'border-[#2A2E38]' : 'border-slate-200'}`}>
      ...
      {(progress || message) && <div className={`px-6 pb-6 space-y-3 text-xs ${muted}`}>
        {progress && <div><div className="flex justify-between mb-1"><span>{progress.phase === 'reading' ? `Lendo ${progress.collection}` : progress.phase === 'deleting' ? `Apagando ${progress.collection}` : `Gravando ${progress.collection}`}</span><strong>{progressPercent}%</strong></div><div className={`h-2 rounded-full overflow-hidden ${isDark ? 'bg-[#2A2E38]' : 'bg-slate-200'}`}><div className="h-full bg-cyan-500 transition-all" style={{ width: `${progressPercent}%` }} /></div></div>}
